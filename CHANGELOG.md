...
- **`redirect-truncate-root-home` fires only on unquoted redirects** (`core.filesystem`): the rule runs on a rule-scoped scan view that blanks the redirect character inside quoted, non-executing text, so `ssh host 'date > ~/stamp'` (a GENUINE remote payload) and `send --body "wrote date > ~/stamp"` (a message body) are content, not a local truncate. **The view is an allowlist and its default is fail CLOSED.** A quoted redirect character is blanked only when every pipeline stage of its segment has a command word on an explicit list of content-bearing, non-executing programs (`orca`, `echo`, `printf`, `jq`, `grep`/`rg`, `sed` without an `e` command or `s///e` flag, `cat`/`tee`/`less`, `git` for a `commit`/`tag`/`notes`/`stash` message, `gh` for an issue/PR `--body`/`--title`, `curl`/`wget` for a `-d`/`--data` payload, and `ssh`/`scp`/`sftp` aimed at a genuine remote host) and that program's own condition holds. Argv-exec wrappers (`sudo`, `doas`, `env`, `timeout`, `nohup`, `nice`, `ionice`, `time`, `command`, `setsid`, `stdbuf`) are transparent — the INNER command word decides, and each wrapper's option scan stops at its first operand, so `sudo grep -i "note: date > ~/stamp" file` is grep's `-i`, not sudo's shell mode. The test applies to the command word ONLY: `orca send --subject python --body "…"` is content because `orca` is the command word.
  Everything else is left completely unmasked and keeps denying at Critical exactly as it did before the rule grew a scan view: unquoted operators (`> ~/x`, `echo x > "/etc/passwd"`, `true && > /etc/passwd`); any unlisted program; a substituted command word (`$SHELL`, `${SHELL}`, `$(which sh) -c '…'`); a quote- or backslash-obfuscated name, which is dequoted the same way the shipped entry point dequotes it (`"sh"`, `s'h'`, `s\h` all resolve to `sh`, which is not a content command); interpreters (`python3 run.py`, `perl -e`, `node -e`, `awk`); string-executing programs (`bash -c`, `sh -c`, `eval`, `xargs`, `script -c`, `su -c`, `expect`, `tmux send-keys`, `find -exec`, `git submodule foreach`, `git config alias.…`, `ansible -m shell`, `nix-shell --run`, `npx -c`, `gdb -ex`, `make`, `sed …/e`); shell-mode wrappers (`sudo -s`/`-i`, `env -S`); a pipe into any of those (`echo '…' | sh`, `… | at now`); and `ssh`/`scp`/`sftp` that is not provably remote — a loopback or own-hostname destination, a substituted host, an unresolvable own hostname (fail closed), any `-o`/`-O` option outside a small safe set (`ProxyCommand`, `LocalCommand`, `PermitLocalCommand`, `ProxyJump`, `Match exec` and unknown keys all run through a LOCAL shell), or a command-shaped `-J`/`-W`/`-L`/`-R`/`-D` value. This host's name is resolved from `/proc/sys/kernel/hostname`, `/etc/hostname`, `$HOSTNAME`, then `uname -n`/`hostname` (portable to macOS), with no new dependency. Engine support: `DestructivePattern::scan_view` (optional, `None` for every other pattern; its documented byte-length invariant is now `debug_assert`ed at the boundary) and `split_command_segment_ranges`. The whole fixture suite is asserted at BOTH layers — the pack API and the shipped path (`normalize_command` + heredoc masking + pack evaluation). No allowlist entries.
- **Breaking (source, pre-1.0 minor bump to 0.5.0)**: `DestructivePattern` gained the required pub field `scan_view: Option<fn(&str) -> Cow<'_, str>>` and its regex field is now `raw_regex`. Migration for downstream struct literals: add `scan_view: None` and rename `regex:` to `raw_regex:`; match through `DestructivePattern::is_match` / `find` (which honour `scan_view`) rather than the raw field. The `destructive_pattern!` macro and external YAML packs are already updated.
