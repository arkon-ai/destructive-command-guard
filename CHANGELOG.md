...
- **`redirect-truncate-root-home` fires only on unquoted redirects** (`core.filesystem`): the rule now runs on a rule-scoped scan view that blanks `>` inside quoted, non-executing text, so `ssh host 'date > ~/stamp'` (a GENUINE remote payload) and `send --body "wrote date > ~/stamp"` (a message body) are content, not a local truncate. Unquoted operators (`> ~/x`, `echo x > "/etc/passwd"`, `true && > /etc/passwd`) still deny at Critical, and so does any segment that runs its quoted text on THIS host: shells and string executors in any position, including behind shell sugar (`bash -c`, `(sh -c ...)`, `{ sh -c ...; }`, `$(sh -c ...)`, `eval`, `xargs`, `script -c`, `su -c`, `runuser -c`, `expect`, `tclsh`, `osascript`, `screen -X`, `tmux send-keys`/`run-shell`, `nsenter`, `unshare`, `chroot`, `parallel`, `at`/`batch`, `systemd-run`, `find -exec`, `awk`/`perl`/`python`/`ruby`/`node`/`lua`/`php`, `... | sh`), shell-mode wrappers (`sudo -s`/`-i`, `env -S`), and `ssh`/`scp`/`sftp` pointed at THIS host (`localhost`, `127.*`, `::1`, `0.0.0.0`, `-o Host=`/`HostName=` naming those, the machine's own hostname read from the kernel, or any `$(...)`/`$VAR` substitution in the host position — fail closed). Scope of the "content" claim: argv-exec wrappers (`sudo`, `env`, `timeout`, `nohup`, `nice`, `time`, `command`, `setsid`, `stdbuf`) are transparent, so `sudo echo "use > ~/file"` and `timeout 30 ssh remote 'date > ~/stamp'` are content; anything else in front of the quoted text is treated as a local executor and keeps denying. Engine support: `DestructivePattern::scan_view` (optional, `None` for every other pattern). No allowlist entries.
