...
- **rm -rf on a shell variable is classified by its binding in the executed command** (`core.filesystem`, transformate WI-3172): `rm -rf "$B"`, `rm -rf ${B}`, `rm -rf -- "${W:?}"` used to be judged by the `$B` substring alone, so scratch-directory cleanup through a variable always denied (rm-rf-general High) while `B=/; rm -rf "$B"` was only High. The rm parser now resolves a pure variable target to the value the same command text binds to it (a top-level `NAME=VALUE` segment, optionally `export`/`local`-led, before the `rm`): a binding in the TMPDIR safe set (`/tmp/`, `/var/tmp/`, `$TMPDIR/`, ...) or a classifiable `$(mktemp ...)` invocation is allowed exactly like the literal; a binding to `/`, `~`, `$HOME`, a system path or a traversal is denied as rm-rf-root-home **Critical** (raised from High); a name bound to a non-safe value anywhere in the command, an unassigned name, a prefix-form assignment (`B=/tmp/x rm -rf "$B"`), a binding the guard cannot classify, or a name touched by `read`/`+=`/`printf -v`/a subshell/`eval` stays on today's rm-rf-general floor. `--` no longer hides the paths after it from either tier (`rm -rf -- /` is now Critical; `rm -rf -- /tmp/x` is allowed). A binding to an absolute path at depth >= 2 below a home directory (`/home/<user>/<a>/<b>`, `/root/<a>/<b>`, `$HOME/<a>/<b>`, `~/<a>/<b>`, no `..` segment, no further expansion) is not a root or home path and is allowed like a temp path; the home itself and its depth-1 children (`~`, `~/dev`, `~/.ssh`, `/root/x`) bound to a variable stay rm-rf-root-home Critical, and the same path typed literally in argv is unchanged (still Critical). No rule added or removed; no severity lowered.
